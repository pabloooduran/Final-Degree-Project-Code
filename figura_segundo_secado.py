...
NOMBRE_SALIDA = "figura_segundo_secado.png"

# ─── Colores y estilos ────────────────────────────────────────────────────────
# Orden alfabético de los CSVs:
#   índice 0 = 3.5 mg/mL  (hilo3.5_... < hilo_... porque '3' < '_')
#   índice 1 = Sin colágeno (hilo_aire)
_pal = sns.color_palette("colorblind", 3)
COLORES   = [_pal[2], "black"]
ETIQUETAS = ["3.5 mg/mL", "Sin colágeno"]
ESTILOS   = [":",    "-"]
ANCHURAS  = [1.5,   2.0]
ALPHAS    = [0.85,  1.0]
ORDEN_LEY = [1, 0]  # Leyenda: Sin colágeno primero
...
def dibujar_tiempo(ax, entradas):
    orden = sorted(range(n), key=lambda i: abs(entradas[i][1].min()), reverse=True)
    for idx in orden:
        x, y = entradas[idx]
        ax.plot(x, y,
                color=COLORES[idx], linestyle=ESTILOS[idx],
